...
@load "checkpoints/model_epoch-1_batch-54.bson" local_model
...
test_acc = accuracy(x_test, y_test, local_model) # 0.9388199261506147
...
loss_plot = plot(y_loss, x_loss, label="", title="Training Loss")

x_acc = accuracy_data[:,1]
y_acc = accuracy_data[:,2]

accuracy_plot = plot(y_acc, x_acc, label="", title="Training Accuracy")
